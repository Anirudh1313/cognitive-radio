...
#define REG_NODE_ADDRESS		 0x33 //node address reg
...
//REG page 100; datasheet - BEACON (page 75 : definition of beacon )
...
int LoRaendPacket()
{
  ...
  // wait for TX done
  uint8_t reg;
   while((reg = readRegister(REG_IRQ_FLAGS) & IRQ_TX_DONE_MASK) == 0)
   {
       LOGD("reg =%x\n", reg);
   }

  // clear IRQ's writeRegister(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_TX);
...
int parsePacket(int size)
{
	...
	if((irqFlags & IRQ_RX_DONE_MASK))// && ((irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK) == 0))
	{
	    if(irqFlags & IRQ_PAYLOAD_CRC_ERROR_MASK)
	    {
	        ...
	        writeRegister(REG_IRQ_FLAGS, readRegister(REG_IRQ_FLAGS) | (0x01 << 5));
	    }
		// received a packet
		_packetIndex = 0;
...
void setNodeAddress()
{
	...
	writeRegister(REG_NODE_ADDRESS, 0x28);
}
...
uint32_t getBitrate()
{
    return readRegister(REG_BITRATE_MSB) << 8 | readRegister(REG_BITRATE_LSB);
}
